...
class CLSEncoder(json.JSONEncoder):
    ...
    def default(self, o):
        if isinstance(o, Tree):
            ...
            return {"__type__": CLSEncoder.tpe(o),
                    "environment": dict((self.constructor_hook(k), [self.constructor_hook(c) for c in v])
            ...
            return {"__type__": CLSEncoder.tpe(o),
                    "repository": dict((self.combinator_hook(c), self.default(t)) for c, t in o.repository),
    ...
    def __call__(self, dct):
        if "__type__" in dct:
            ...
            if tpe == CLSDecoder.tpe(Tree):
                ...
                return Combinator(target=dct["target"], combinator=self.combinator_hook(dct["combinator"]))
            elif tpe == CLSDecoder.tpe(Apply):
                return Apply(target=dct["target"],
                ...
                return Constructor(name=self.constructor_hook(dct["name"]), arg=dct["arg"])
            elif tpe == CLSDecoder.tpe(Subtypes):
                return Subtypes(environment =
                                dict((self.constructor_hook(k),
                                      {self.constructor_hook(st) for st in v})
                                     for k, v in dct["environment"].items()))
            elif tpe == CLSDecoder.tpe(InhabitationResult):
                return InhabitationResult(targets=dct["targets"], rules={r for r in dct["rules"]})
            elif tpe == CLSDecoder.tpe(FiniteCombinatoryLogic):
                return FiniteCombinatoryLogic(
                    repository=dict((self.combinator_hook(k), v) for k, v in dct["repository"].items()),
